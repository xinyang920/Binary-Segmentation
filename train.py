...
from net import MyNet as mynet
# ----------------------- load package ----------------------- #
...
def validate(model, val_loader,nums=250):
    model.train(False)
    avg_mae = 0.0
    with torch.no_grad():
...
def train(Dataset, Network, config_dict):
    ...
    for epoch in range(start_epoch, cfg.epoch):
        ...
        for step, (image, mask, edge) in enumerate(loader):
            ...
            sw.add_scalars('loss', {'loss': loss.item(),'loss1': loss1.item(),'loss2': loss2.item(),
                                    'loss3': loss3.item(),'loss4': loss4.item()}, global_step=global_step)
            if step % 10 == 0:
        ...
        if config_dict["val"]:
            if epoch > cfg.epoch*config_dict["val_ratio"]-1:
                mae = validate(net, val_loader, config_dict["val_total"])
                print('MAE:%s' % mae)
                if mae < min_mae:
                    min_mae = mae
                    best_epoch = epoch + 1
    ...
    def save(self):
        ...
        with torch.no_grad():
            for image, mask, shape, name in self.loader:
                ...
                out, ax1, ax2, ax3 = self.net(image)
                # print(shape)
